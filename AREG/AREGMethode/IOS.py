...
# {'Process':SegProcess,'Parameter':parameter_seg},{'Process':PreOrientProcess,'Parameter':parameter_pre_aso},
        ...
        display = {'CrownSegmentationcli':DisplayCrownSeg(number_scan_toseg,kwargs['logPath']),
                    ...
                    'PRE_ASO_IOS': DisplayAREGIOS(numberscan if len(jaw) ==1 else int(numberscan/2) , jaw,kwargs['logPath'] ),
                    # 'SEMI_ASO_IOS': DisplayAREGIOS(numberscan if len(jaw) ==1 else int(numberscan/2) , jaw,kwargs['logPath'] )
                    }
#
...
class Semi_IOS(Auto_IOS):
    ...
    def Process(self, **kwargs):
        ...
        OrientProcess = slicer.modules.semi_aso_ios
        numberscan = self.NumberScan(kwargs['input_folder'])
        return [{'Process':OrientProcess,'Parameter':parameter}], { 'SEMI_ASO_IOS': DisplayAREGIOS(numberscan if len(jaw) ==1 else int(numberscan/2) , jaw,kwargs['logPath'] )}
